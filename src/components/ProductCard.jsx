import { platformLabels, platformTextColors } from "@/data/groceries";
  ...
  return (
    <div className="group rounded-xl border border-border bg-card p-5 shadow-card transition-all duration-300 hover:shadow-card-hover hover:-translate-y-0.5">
      ...
      <div className="space-y-2">
        {item.prices.map((pp) => {
          ...
          return (
            <div
              key={pp.platform}
              className={`flex items-center justify-between rounded-lg px-3 py-2 text-sm transition-colors ${isCheapest ? 'bg-secondary' : 'bg-muted/50'} ${!pp.inStock ? 'opacity-50' : ''}`}
            >
      ...
      <div className="mt-4 flex items-center justify-between">
        {savings > 0 && (
          <span className="text-xs font-medium text-primary">Save up to ₹{savings}</span>
        )}
